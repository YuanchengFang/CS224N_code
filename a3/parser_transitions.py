...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        # 3. Right Arc
        if transition == 'S':
            pop_word = self.buffer.pop(0)
            self.stack.append(pop_word)
        elif transition == 'LA':
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        # 3.1 Take the first batch size parses in unfinished parses as a minibatch
        minibatch_parses = unfinished_parses[:batch_size] if len(
            unfinished_parses) > batch_size else unfinished_parses[:]
        unfinished_parses = unfinished_parses[batch_size:] if len(
            unfinished_parses) > batch_size else []
        minibatch_index = list(range(len(minibatch_parses)))
        minibatch_dependencies = [None] * len(minibatch_parses)
        while minibatch_parses:
            # 3.2 Use the model to predict the next transition for each partial parse in the minibatch
            minibatch_transitions = model.predict(minibatch_parses)
            removed_index = []
            for i in range(len(minibatch_parses)):
                # 3.3 Perform a parse step on each partial parse in the minibatch with its predicted transition
                minibatch_parses[i].parse_step(minibatch_transitions[i])
                # 3.4 Remove the completed (empty buffer and stack of size 1) parses from unfinished parses
                if (not minibatch_parses[i].buffer) and len(minibatch_parses[i].stack) == 1:
                    minibatch_dependencies[minibatch_index[i]
                                           ] = minibatch_parses[i].dependencies
                    removed_index.append(i)
            for index in sorted(removed_index, reverse=True):
                del minibatch_parses[index], minibatch_index[index]
        dependencies.extend(minibatch_dependencies)
    # END YOUR CODE
