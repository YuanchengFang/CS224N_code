...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.parameter.Parameter(
            torch.Tensor(self.embed_size * self.n_features, hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight.data)
        self.embed_to_hidden_bias = nn.parameter.Parameter(
            torch.Tensor(hidden_size))
        nn.init.uniform_(self.embed_to_hidden_bias.data)
        self.dropout = nn.Dropout(p=self.dropout_prob)
        self.hidden_to_logits_weight = nn.parameter.Parameter(
            torch.Tensor(hidden_size, n_classes))
        nn.init.xavier_uniform_(self.hidden_to_logits_weight.data)
        self.hidden_to_logits_bias = nn.parameter.Parameter(
