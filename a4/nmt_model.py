...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(
            input_size=hidden_size + embed_size, hidden_size=hidden_size, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        output, (hiddens, cells) = self.encoder(packed_X)
        enc_hiddens, length = pad_packed_sequence(output)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        last_hidden = torch.cat([hiddens[0, :, :], hiddens[1, :, :]], dim=1)
        last_cell = torch.cat([cells[0, :, :], cells[1, :, :]], dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Tensor Squeeze:
        # https://pytorch.org/docs/stable/torch.html#torch.squeeze
        hiddens, cells = self.decoder(Ybar_t, dec_state)
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(hiddens, dim=2))
        ...
        # a_t = torch.bmm(enc_hiddens.permute(0, 2, 1), alpha_t.unsqueeze(dim=2))
        # U_t = torch.cat([a_t.squeeze(dim=2), hiddens], dim=1)
        # V_t = self.combined_output_projection(U_t)
        # O_t = self.dropout(torch.tanh(V_t))
        alpha_t = torch.unsqueeze(
            F.softmax(e_t, dim=1), dim=1)			# (batchsize, seqlen)
        # (batchsize, 2 * hiddensize)
        a_t = torch.squeeze(torch.bmm(alpha_t, enc_hiddens), dim=1)
        # (batchsize, 3 * hiddensize)
        U_t = torch.cat((a_t, hiddens), dim=1)
        V_t = self.combined_output_projection(
            U_t)  					# (batchsize, hiddensize)
        O_t = self.dropout(torch.tanh(V_t))
        # END YOUR CODE
