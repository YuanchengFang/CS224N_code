...
def naiveSoftmaxLossAndGradient(
    ...
    # to integer overflow.
    soft = softmax(outsideVectors @ centerWordVec)
    loss = -np.log(soft[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + \
        (outsideVectors.T @ soft)
    soft[outsideWordIdx] = soft[outsideWordIdx] - 1
    gradOutsideVecs = np.outer(soft, centerWordVec)
    # END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    # END YOUR CODE
    # negSampleWordIndices = np.array([1, 2])
    uv_dot = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss = -np.log(uv_dot) \
        - np.sum(np.log(sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec)))
    gradCenterVec = (uv_dot - 1) * outsideVectors[outsideWordIdx] + \
        np.sum((1 - sigmoid(-outsideVectors[negSampleWordIndices] @
                            centerWordVec)).reshape((K, 1)) * outsideVectors[negSampleWordIndices], axis=0)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (uv_dot - 1) * centerWordVec
    for ind in negSampleWordIndices:
        gradOutsideVecs[ind] += (1-sigmoid(-np.dot(outsideVectors[ind],
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:
        ...
        _loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(
            centerWordVec=center_word_vector, outsideWordIdx=outside_word_id, outsideVectors=outsideVectors, dataset=dataset)
        loss += _loss
